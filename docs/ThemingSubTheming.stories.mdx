...
Distinct color palettes are used to reflect these special occasions but it's not
always the case that we want to have completely new brand (new radius, spacing, etc) values for the pages reflecting these themes.
This is where sub theming comes in.

Sub theming allows us to target certain properities of a theme to change while still keeping the values of the other theme properties.
...
Additionally, `ProgressBars` used outside the context of this sub-theme will continue to use the brand colors of the main theme.

Likewise, if there are multiple elements to a page, we can set a certain element to have a sub-theme (a contentTray with the Shark Week sub-theme)
...
> Otherwise, if it does have a `subTheme` applied, ie 'sharkWeek', then the 'sharkWeek' sub-theme will overwrite the globally set '4thOfJuly' sub-theme.
...
The above code will render two `ProgressBars`, each utilizing a different sub-theme, as shown below.
