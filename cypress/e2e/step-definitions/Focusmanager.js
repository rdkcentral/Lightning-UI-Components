     ...
     Then(
      ...
      (width, height, pageName) => {
        ...
        cy.get(pageObject.rowElements)
          .each($element => {
            cy.wrap($element).invoke('attr', 'style').should('contain', `width: ${width}px`);
            cy.wrap($element).invoke('attr', 'style').should('contain', `height: ${height}px`);
          })
    }
  );
   ...
   */
    Then(
     ...
     (pageName) => {
      ...
      cy.get(pageObject.rowElements)
      .each($element => {
        cy.wrap($element).invoke('attr', 'style').should('contain', 'left: 0px');
      })
    }
  );
