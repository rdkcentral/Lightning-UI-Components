...
| icon           | image         | false    | null                                   | icon to display in input                                                                                                                                             |
| iconColor      | number        | false    | Xfinity theme (theme.palette.grey[5])  | icon color for input unfocused state                                                                                                                                 |
| iconFocusColor | number        | false    | Xfinity theme (theme.palette.grey[90]) | icon color for input focused state                                                                                                                                   |
| caption        | string\|array | false    | -                                      | used as the "content" in an [InlineContent](?path=/docs/inlinecontent--basic) component and generates the caption line                                               |
| listening      | boolean       | false    | false                                  | When true, input will accept changes from `clear`, `insert`, and `backspace` methods. When false, effects of those methods will be ignored and not affect the input. |
