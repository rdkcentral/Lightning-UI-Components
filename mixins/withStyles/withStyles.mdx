...
https://github.comcast.com/Lightning/lightning-ui/packages/@lightning/ui-core/src/components/withStyles/withStyles.js
...
So far, we have seen `withStyles` implementations using a `styles` object as an argument. Another way to implement `withStyles` is to pass a `styles` function that accepts a `theme` object as an argument. If no `theme` is provided, the default **Xfinity** theme will be used. See the [Themes](?path=/docs/styles-themes--page) documentation for more info on theme structure.
