...
Alternatively, an array of components and `style` keys may be provided as `content`. Styles provided here will override the ones set using the [`withStyles` mixin](#styling) for it's section of text:
...
If the array of items contains rows/columns with different heights and/or widths set autoResizeWidth and autoResizeHeight to true so that the scroll bar is of the appropriate height.
