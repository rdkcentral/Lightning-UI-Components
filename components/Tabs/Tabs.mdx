...
Icon sizes can be defined in one of two ways: `iconHeight`/`iconWidth` or `iconSize`. The `iconSize` property is an enum of `small`, `medium`, and `large`, which align with the icon sizes set in the given theme (default is Xfinity). For non-standard icon sizes, use `iconHeight` and `iconWidth`.
