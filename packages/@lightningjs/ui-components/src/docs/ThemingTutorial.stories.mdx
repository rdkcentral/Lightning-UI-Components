...
All components in LUI extend a single `Base` component, which you can read about [here](../?path=/story/docs-base-component--page). This Base component is wrapped in helpful mixins like [`withThemeStyles`](../?path=/docs/utilities-withthemestyles--with-theme-styles), [`withHandleKey`](../?path=/docs/utilities-withhandlekey--with-handle-key), [`withUpdates`](../?path=/docs/utilities-withupdates--with-updates), and [`withTags`](../?path=/docs/utilities-withtags--with-tags). It is essential to utilize this Base component in your applications in order to get the full benefits of theming.
