...
| size       | string   | false    | sm        | Used to determine the with of the key. Accepts: `sm`, `md`, `lg`, `xl`, `xxl`.                                                                                                                                        |
...
| keySpacing | number   | false    | 0         | The horizontal spacing between each key in a [Keyboard](?path=/docs/components-keyboard--docs). This value is factored intot he width of the key so that it aligns with with the borders of other keys in a Keyboard. |
