...
import * as NavigationManagerStories from './NavigationManager.stories';

<Meta of={NavigationManagerStories} />
...
`direction` propety set in [FocusManager](?path=/docs/components-focusmanager--row). The properties it maps to are as follows:
