...
NavigationManager is a is a subclass of [FocusManager](?path=/docs/navigation-focusmanager--rows),
and parent class of [Row](?path=/docs/navigation-row--row) and [Column](?path=/docs/elements-column--basic).
