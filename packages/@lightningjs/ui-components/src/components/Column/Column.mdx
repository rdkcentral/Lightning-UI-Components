...
The `Column` component extends FocusManager, so you need to make sure that `_getFocused` returns the `Column` component.
...
| ------------- | ------------------------------------------------------- | -------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| items         | <DocsLink id="lng.Component">lng.Component[]</DocsLink> | false    | undefined | Array of components to be listed in the column.                                                                                                  |
| onScreenItems | array                                                   | true     | undefined | List of items that are currently on screen within the bounds of the container.                                                                   |
| plinko        | boolean                                                 | false    | false     | When `true`, the component will determine the nearest horizontal index of the next focused item's children. Useful if Column has Rows as items.. |
...
| previous | <DocsLink id="lng.Component">lng.Component[]</DocsLink> | false    | -       | The previously selected item(s). Required only if this.plinko is true. |
