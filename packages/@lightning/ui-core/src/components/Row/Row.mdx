...
| startLazyScrollIndex | number                                                  | false    |           | when `lazyScroll` is `true`, index of item in `items`, and items thereafter, at which lazy scrolling should occur at (always scroll functionality will take place before this index)                                                 |
| stopLazyScrollIndex  | number                                                  | false    |           | when `lazyScroll` is `true`, index of item in `items`, and items preceding, at which lazy scrolling should occur (always scroll functionality will take place after this index)                                                      |
